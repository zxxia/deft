...
def plot_jct_vs_time(model_A_log, model_B_log, model_A_name, model_B_name, save_dir):
    min_t = min(model_A_log['inf_start_timestamp_ns'].iloc[0],
                model_B_log['inf_start_timestamp_ns'].iloc[0])
    max_t = max(model_A_log['inf_end_timestamp_ns'].iloc[-1],
                model_B_log['inf_end_timestamp_ns'].iloc[-1])
    fig = plt.figure(figsize=(16, 8))
    ax = plt.gca()

    ax.plot((model_A_log['inf_end_timestamp_ns'] - min_t) / SEC_IN_NS,
            model_A_log['jct_ms'], 'o-', label=model_A_name)
    ax.plot((model_B_log['inf_end_timestamp_ns'] - min_t) / SEC_IN_NS,
...
def main():
    ...
    model_A_jct_log = pd.read_csv(args.model_A_jct_log)[1:]
    model_B_jct_log = pd.read_csv(args.model_B_jct_log)[1:]
